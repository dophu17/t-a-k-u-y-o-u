...
class LC_Page_Products_Detail extends LC_Page_Ex
{
    ...
    public function action()
    {
        ...
        $explodeWpPostId = explode(',', $this->arrProduct['wp_post_id']);
        $arrWpPostId = array();
        $col = '*'; 
        $from = 'wp_posts';   
        $where = '';
        $arrWpPost = array();
        //set array post wordpress
        foreach ( $explodeWpPostId as $item ) {
            if ( $item && $item > 0 && is_numeric($item) ) {
                $where = 'wp_posts.ID = ' . $item;
                $result = $objQuery->select($col, $from, $where);

                //get thumbnail
                $colThumb = 'wp_postmeta.*, wp_posts.id, wp_posts.guid';
                $fromThumb = 'wp_postmeta INNER JOIN wp_posts ON wp_postmeta.meta_value = wp_posts.id';
                $whereThumb = "wp_postmeta.post_id = " . $result[0]['id'] . " AND wp_postmeta.meta_key = '_thumbnail_id'";
                $thumbs = $objQuery->select($colThumb, $fromThumb, $whereThumb);
                $result[0]['thumb'] = $thumbs[0];
                //end get thumbnail

                $arrWpPost[] = $result[0];
            }
        }
        $this->arrProduct['wp_posts'] = $arrWpPost;
        $this->url_noimage = 'http://' . $_SERVER['HTTP_HOST'] . IMAGE_SAVE_URLPATH . 'noimage_main.png';
        //end get post from wordpress
