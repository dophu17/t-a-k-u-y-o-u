...
require_once CLASS_REALDIR . 'service/WordpressService.php';
...
class LC_Page_Products_List extends LC_Page_Ex
{
    ...
    public function action()
    {
        ...
        $this->arrProducts  = $this->combineWordpressData($this->arrProducts);
    ...
    public function lfGetProductsList($searchCondition, $disp_number, $startno, &$objProduct)
    {
        ...
        if ( $arrProducts && count($arrProducts) > 0 ) {
            foreach ( $arrProducts as $index => $item ) {
                $arrProducts[$index]['url_main_image'] = 'http://' . $_SERVER['HTTP_HOST'] . IMAGE_SAVE_URLPATH . $item['main_image'];
            }
        }
        //end set full url for avatar
    ...
    protected function combineWordpressData($products)
    {
        ...
        foreach ($products as $key => &$product) {
            ...
            $wpPostId = $product['wp_post_id'];

            if ($wpPostId) {
                $product['wordpress_post'] = $wordpressService->getPost($wpPostId);
            } else {
                $product['wordpress_post'] = array();
            }
        }
