...
class LC_Page_Admin_Products_Product extends LC_Page_Admin_Products_Ex
{
    ...
    public function action()
    {
        ...
        switch ($mode) {
            ...
            case 'edit':
                ...
                // check wordpress input post id
                if ( $_POST['wp_post_id'] && $_POST['wp_post_id'] != '' ) {
                    $explodeWpPostId = explode(',', $_POST['wp_post_id']);
                    $arrWpPostId = array();
                    //convert array to true, check post id
                    foreach ( $explodeWpPostId as $item ) {
                        if ( $item && $item > 0 && is_numeric($item) ) {
                            //check post id in wp
                            $where = 'wp_posts.ID = ' . trim($item);
                            $result = $objQuery->select($col, $from, $where);
                            if ( $result && count($result) > 0 ) {
                                $arrWpPostId[] = trim($item);
                            }
                        }
                    }
                    //set field for insert/update to table "wp_post_id"
                    foreach ( $arrWpPostId as $key => $postId ) {
                        if ( $key > 2 ) break;
                        if ( $key == count($arrWpPostId) - 1 || $key == 2 ) {
                            $fieldWpPostId .= $postId;
                        } else {
                            $fieldWpPostId .= $postId . ',';   
                        }
                    }
                } else {
                    //get 3 post from wp by random
                    ...
                    if ( count($result) > 0 ) {
                        foreach ( $result as $key => $item ) {
                            if ( $key == count($result) - 1 || $key == 2 ) {
                                $fieldWpPostId .= $item['id'];
                            } else {
                                $fieldWpPostId .= $item['id'] . ',';   
                            }
                        }
                ...
                $_POST['wp_post_id'] = $fieldWpPostId;
                // end check wordpress post id
    ...
    public function lfInitFormParam(&$objFormParam, $arrPost)
    {
        ...
        $objFormParam->addParam('wp_post_id', 'wp_post_id', STEXT_LEN, '', array());//'NUM_CHECK', 'MAX_LENGTH_CHECK'
...
__EOF__;
    ...
    public function lfRegistProduct(&$objUpFile, &$objDownFile, $arrList)
    {
        ...
        $sqlval['wp_post_id'] = $arrList['wp_post_id'];
